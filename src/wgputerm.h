...
generic C core, so it must fully define struct wgpu_output, struct
wgpu_display_info, and the FRAME_* accessor macros the shared code expects.
It mirrors the generic skeleton of pgtkterm.h but with backend-neutral types
(no Gdk/Gtk/cairo) since the wgpu backend talks raw Wayland.
...
struct wgpu_display_info
{
  ...
  struct wgpu_bitmap_record *bitmaps;
...
/* Per-frame backend state, hung off f->output_data.wgpu.  */
struct wgpu_output
{
...
#define FRAME_X_OUTPUT(f)         ((f)->output_data.wgpu)
#define FRAME_OUTPUT_DATA(f)      FRAME_X_OUTPUT (f)
#define FRAME_WGPU_OUTPUT(f)      FRAME_X_OUTPUT (f)
...
#define FRAME_X_WINDOW(f)         (FRAME_X_OUTPUT (f)->wgpu_frame)
...
/* Chain of all wgpu displays (named x_display_list to satisfy generic code
   that pokes the backend's display list, e.g. display_available).  */
extern struct wgpu_display_info *x_display_list;

extern struct wgpu_display_info *wgpu_term_init (Lisp_Object display_name);
extern struct wgpu_display_info *check_x_display_info (Lisp_Object object);
