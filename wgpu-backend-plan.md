# wgpu backend for Emacs — design & execution plan
...
- **Render API:** `wgpu` (Vulkan/GL/Metal backends).
...
- **Validation:** offscreen wgpu render + pixel readback, diffed against golden
  PNGs. Deterministic, no compositor, CI-able. Triggered from Lisp via
  `(wgpu-dump-frame PATH)`.
...
- **Build/link:** `cargo` staticlib (`libwgpu_backend.a`) + `cbindgen`-generated
  header, linked into the `emacs` binary. `configure` flag `--with-wgpu`.
...
- **Backend id:** `wgpu`. Macro `HAVE_WGPU`, `window_system == "wgpu"`, output
  method `output_wgpu`, Lisp symbol `Qwgpu`.
- **C side (thin shims):** `src/wgputerm.c`, `src/wgputerm.h`, `src/wgpufns.c`
  (frame/Lisp glue), `src/wgpufont.c` (font driver). Hold no drawing logic —
  only translation to FFI calls. Structurally modeled on `pgtkterm.c`
  (`pgtk_create_terminal` at `pgtkterm.c:4812`) but ~10x thinner.
- **Rust side:** crate `rust/wgpu-backend/` → staticlib `libwgpu_backend.a`.
  Owns Wayland, wgpu, glyph atlas, render thread, input decode.
- **Generated header:** `src/wgpu_ffi.h` (cbindgen, Rust→C exports). Hand-written
  `wgpu_callbacks.h` for the small set of C functions Rust calls back into.
...
```
  ...
  │ owns wgpu device/queue/      │   │ smithay loop: keyboard/pointer →    │
...
Invariants:
- All wgpu state lives on the render thread (no `Surface`/`Device` across
  threads).
...
populated in `wgpu_create_terminal` (analog of `pgtkterm.c:4812`); each hook body
is a 1–3 line forwarder to a `wgpu_*` FFI call.

Rust → C (cbindgen):
- Lifecycle: `wgpu_backend_init`, `wgpu_terminal_create`, `wgpu_frame_create`,
  `wgpu_frame_destroy`, `wgpu_shutdown`.
- Cmd recording (one per RIF hook): `wgpu_clear_area`, `wgpu_draw_glyphs`,
  `wgpu_fill_rect`, `wgpu_draw_rect`, `wgpu_scroll`, `wgpu_draw_cursor`,
  `wgpu_draw_fringe_bitmap`, `wgpu_draw_window_divider`, …
- Glyph atlas: `wgpu_atlas_get_or_raster(font_key, glyph, subpixel)`.
- Present: `wgpu_frame_present`.
- Input: `wgpu_backend_poll_events(buf, max)`, `wgpu_backend_wakeup_fd`.
- Misc: `wgpu_set_scale`, `wgpu_resize`, `wgpu_dump_frame(path)` (test hook).

C → Rust callbacks: glyph raster bridge (`wgpu_cb_raster_glyph`), color/resource
queries. Kept minimal.

## Rendering pipeline (Rust)

- Command buffer: `enum WgpuCmd { Glyphs, FillRect, Rect, Scroll, Cursor,
...
- Reuse Emacs shaping: `wgpufont` driver layers on existing FreeType+HarfBuzz
...
- smithay keyboard → `xkbcommon` keymap → keysym + mods. Port modifier/keysym →
  ...
  lock-free queue.
- `read_socket_hook` calls `wgpu_backend_poll_events`, converts to Emacs input
...
- Primary gate (deterministic, no compositor): render thread can target an
  offscreen texture; `(wgpu-dump-frame PATH)` forces redisplay → offscreen
  replay → readback → PNG.
- Golden tests: elisp scripts in `test/manual/wgpu/` set up known
  buffer/face/scale → `wgpu-dump-frame` → compare to committed golden PNG with
  per-pixel tolerance + diff image on failure. Goldens in
  `test/manual/wgpu/golden/`.
- Rust unit tests: atlas packing, cmd encoding, xkb mapping, scroll math —
  `cargo test`, no GPU.
- Latency regression: run `test/manual/pgtk-keybench.el` on `wgpu`; compare
...
- `configure.ac`: `--with-wgpu` → `window_system=wgpu`, define `HAVE_WGPU` +
  `HAVE_WINDOW_SYSTEM`, set `WGPU_OBJ="wgputerm.o wgpufns.o wgpufont.o"`, locate
...
- `src/Makefile.in`: rule runs `cargo build --release` (in `guix shell`) →
  `libwgpu_backend.a`; link it + `-lwayland-client -lxkbcommon` (+ wgpu system
  deps: vulkan-loader/libdrm/libgbm). cbindgen pre-build step → `wgpu_ffi.h`.
  (Seams: `Makefile.in:325` PGTK_OBJ, `:469` link list, `:573` LIBES.)
- Core: add `output_wgpu` (`termhooks.h:57`), `Qwgpu` (`frame.c:7258`),
...
- **M0 — Scaffold/build.** Crate + staticlib + cbindgen + `--with-wgpu` +
  Makefile/Guix wiring; `wgpu_create_terminal` stub registers hooks. Gate:
  builds in guix shell; `cargo test` green; `emacs` links.
- **M1 — Window + clear color.** smithay window + wgpu surface; present solid
  color; offscreen path. Gate: solid-color golden via `wgpu-dump-frame`.
- **M2 — Text + cursor.** Persistent texture, glyph atlas, `wgpufont` draw,
...
- Present latency / Mailbox availability varies → presentation-time + measure;
  Fifo fallback.
- Guix vendoring of wgpu's large crate tree → `cargo vendor`; pin versions.
...
- Whether `wgpufont` can fully reuse `sfntfont`/`ftfont` open/shape or needs a
  thin fork.
